...
bool TX60L::ForwardKinematics(std::vector<double> jnts, std::vector<double> &pos){
    ...
    pos.push_back(RADIAN_2_DEGREE(Rx));
    pos.push_back(RADIAN_2_DEGREE(Ry));
    pos.push_back(RADIAN_2_DEGREE(Rz));
...
bool TX60L::Move(std::vector<double> pos){
	if(!mIsLoggedIn)
		return false;
	_ns6__moveJC * moveJC = new _ns6__moveJC();
	
	//initialize the frame
	moveJC->frame = new ns6__Frame();
	moveJC->frame->px=pos[0];
	moveJC->frame->py=pos[1];
	moveJC->frame->pz=pos[2];
	SetFrameFromRxRyRz(moveJC->frame, pos[3], pos[4], pos[5]);
	...
	moveJC->mdesc = md;
	_ns6__moveResponse * moveResponse = new _ns6__moveResponse();
	mCS8ServerV3->moveJC(moveJC, moveResponse);
	delete moveJC;
	delete moveResponse;
	return true;
}
